class Collection < ActiveRecord::Base
  ...
  def self.get(slug)
    ...
    if c
      ...
      generated = []
      Article.with_article_type(c.article_type).with_category(c.category).with_series(c.series).flagged_as(c.flag).sort_by(c.order).with_limit(c.limit).published.all.each do |a|
        generated << a.current
      end
